...
def epipolarlines(img1, img2, pts1, pts2):
    ...
    def findAndMatchFeatures(img1, img2):
        ...
        cv2.resizeWindow("Keypoint matches", 1000, 500)
...
boards = find_board_config("", board_file="charuco_small.yaml").values()
board_detection_base = detect.detect_image(frameBase, boards)[0]
board_detection_match = detect.detect_image(frameMatch, boards)[0]

# find common corners

common_corner_ids = [id for id in board_detection_base["ids"] if id in board_detection_match["ids"]]
pts_base = np.array([pt for pt, id in zip(board_detection_base["corners"], board_detection_base["ids"]) if id in common_corner_ids])
pts_match = np.array([pt for pt, id in zip(board_detection_match["corners"], board_detection_match["ids"]) if id in common_corner_ids])
