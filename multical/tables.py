...
def extract_pose(points, board, camera):# function used to extract board poses given image points and camera data (intrinsic, distortion coeffs, but not pose bcause we don't have that yet)
    detections = sparse_points(points)
    poses = board.estimate_pose_points(camera, detections)#[rvec,tvec] list to define pose relative to camera
...
def map_table(f, point_table, boards, cameras):
    return [[[f(points, board, camera) # gets position of board relative to camera
              for points, board in zip(frame_points._sequence(), boards)] # for the images points of one board and the board data
             for frame_points in points_camera._sequence()] # for the points of each image
            for points_camera, camera in zip(point_table._sequence(), cameras)]# for each camera and their detected image points


def make_pose_table(point_table, boards, cameras):
    poses = map_table(extract_pose, point_table, boards, cameras)# for each camera, for each image, for each board, board position relative to camera (computed from rvec,tvec)
...
def initialise_poses(pose_table, camera_poses=None):# pose_table for each camera, for each image, for each board, board position relative to camera (computed from rvec,tvec)
