...
class PathOpts:
    ...
    camera_pattern: Optional[str] = None  # Camera path pattern example "{camera}/extrinsic"
...
class OptimizerOpts:
    ...
    loss: str = choice('linear', 'soft_l1', 'huber', 'arctan',
                       default='linear')  # Loss function to use in bundle adjustment
    ...
    auto_scale: Optional[
        float] = None  # Threshold for auto_scale to reduce outlier influence (factor of upper quartile of reprojection error) - requires non-linear loss
...
def run_with(command_type):
    ...
    program = parser.parse_args() # program contains arguments for application
